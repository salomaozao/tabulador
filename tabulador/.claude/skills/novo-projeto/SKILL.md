---
name: novo-projeto
description: Cria um projeto novo no Tabulador de respostas abertas a partir de uma planilha de pesquisa (.xlsx no formato "plano", com um cabeçalho e uma coluna por pergunta), seguindo um processo em etapas com travas. O rascunho é gerado sem IA, a pessoa confirma o pedido e o validador roda o pipeline real. Use quando pedirem para "criar projeto", "cadastrar pesquisa nova no tabulador", "configurar projeto.json" ou "preparar categorização de uma base nova".
---

# Novo projeto no Tabulador
...
Rode sempre na pasta `tabulador` (em `jumppi`, ao lado de `SESI_cat`), com o Python do ambiente do tabulador:
