...
| **[PEND-07](pendencias/07_base_inteira_na_nuvem.md)** | **Avaliar guardar a base inteira (não só a revisão) na nuvem, para rodar só o Tabulador sem pasta de projeto local** | Gabriel Nascimento | Baixa | `Aberto` | Ideia do Gabriel: subir também a base (planilha-fonte/base processada), não só o loop de revisão, para o Turso — assim bastaria rodar o `tabulador` (a pasta `SESI_cat` poderia até ficar dentro dele) sem precisar sincronizar outra pasta de projeto. Ainda não decidido; avaliar tradeoffs antes. | [Abrir](pendencias/07_base_inteira_na_nuvem.md) |
...
| **[PEND-09](pendencias/09_identificacao_e_sidebar_presenca.md)** | **Identificação de quem está logado (pessoa ou IA) e sidebar mostrando onde cada um está no projeto** | Gabriel Nascimento | Baixa | `Aberto` | Ideia do Gabriel: hoje o Tabulador identifica quem está usando a máquina pelo login do Windows (config.USUARIO, usado só para avisar de edição simultânea numa mesma pergunta — ver a presença por pergunta implementada em 2026-09-24). Falta: (1) uma identificação explícita na tela inicial — a pessoa se apresenta (ou diz que é uma IA, ex. 'Claude', 'Codex') em vez de confiar só no login do SO; (2) uma sidebar à direita mostrando, para todo o projeto, onde cada pessoa/IA está agora (em qual pergunta, ou no painel/resultados), não só um aviso dentro da pergunta aberta. | [Abrir](pendencias/09_identificacao_e_sidebar_presenca.md) |
