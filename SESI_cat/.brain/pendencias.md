...
| **[PEND-05](pendencias/05_janela_aprovacao_dinamica.md)** | **Janela dinâmica para aprovar categorias (perfil informativo) + supervisor na interface** | Gabriel Nascimento | Media | `Aberto` | Levar o fluxo do supervisor.py (IA classifica, um segundo modelo audita, aceita sozinho o que tem confiança >= 0,70 com auditoria concordando) para a interface, com uma janela de aprovação que mostre por categoria o perfil de quem citou (NPS, satisfação, retenção, escola) e amostras, para aprovar em bloco. Fazer em commit separado. | [Abrir](pendencias/05_janela_aprovacao_dinamica.md) |
| **[PEND-06](pendencias/06_conferir_quadros_aprovados_pelo_claude.md)** | **Conferir quadros de categorias aprovados pelo Claude (Q32/Q33 unificado, Q35) e a fila de revisão** | Gabriel Nascimento | **Alta** | `Aberto` | Na sessão de 23/09 o Claude aprovou quadros sem o pesquisador: Q32 e Q33 com um quadro único de 9 categorias (inclui 'Insatisfação geral' e 'Remete a resposta anterior'; o quadro antigo da Q32 está em output/_backup) e Q35 (14 categorias). Conferir os quadros, revisar as pendentes de cada pergunta e aprovar a codificação para gerar a planilha final. | [Abrir](pendencias/06_conferir_quadros_aprovados_pelo_claude.md) |
