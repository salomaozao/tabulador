...
def _rodar_pipeline(mod, pasta: Path):
    ...
    anterior, env_antes = getattr(config, "PROJETO", None), os.environ.get("CATEGORIZADOR_OUTPUT")
    registro_original = projetos._registro
    tmp = tempfile.mkdtemp(prefix="categorizador_validacao_")
    with _trava:
        try:
            os.environ["CATEGORIZADOR_OUTPUT"] = tmp
            ...
            if env_antes is None:
                os.environ.pop("CATEGORIZADOR_OUTPUT", None)
            else:
                os.environ["CATEGORIZADOR_OUTPUT"] = env_antes
...
def validar(pasta, exigir_conferencia: bool = True) -> dict:
    ...
    try:
        ...
        erros.append(f"A leitura pelo categorizador falhou: {type(e).__name__}: {e}")
        return {**res, "valido": False}
