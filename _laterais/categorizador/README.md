# Categorizador de respostas abertas — categorização, codebook e cruzamentos
...
1. Dê dois cliques em **`iniciar.bat`** (ou, no SESI, em `SESI_cat\Abrir Categorizador.bat`, que já
...
Variáveis de ambiente úteis: `OPENAI_MODEL` (padrão `gpt-4.1`), `CATEGORIZADOR_PARALELO` (chamadas
simultâneas, padrão 4), `ASSERTIVA_LOTE` (respostas por chamada, padrão 30), `CATEGORIZADOR_MAX_INDUCAO`
(máx. de respostas únicas lidas ao propor categorias, padrão 1500 — acima disso usa amostra),
`CATEGORIZADOR_PORTA` (padrão 5000).
