...
(Claude Code com haiku: ~1,3 mil tokens, ~US$ 0,002). Só roda com CATEGORIZADOR_TESTE_CLI=1:
  set CATEGORIZADOR_TESTE_CLI=1 && python -m unittest tests.test_cli -v
...
_TMP = tempfile.mkdtemp(prefix="categorizador_cli_")
os.environ["CATEGORIZADOR_OUTPUT"] = _TMP
...
REAL = os.getenv("CATEGORIZADOR_TESTE_CLI") == "1"
MODELO_BARATO = {"cli_claude": "haiku", "cli_codex": None, "cli_gemini": "gemini-2.5-flash"}
...
class TestBotaoTestar(unittest.TestCase):
    ...
    def test_ok(self):
        ...
        self.assertEqual((r.status_code, r.get_json()), (200, {"ok": True, "modelo": "haiku"}))

    def test_erro_do_programa_vira_mensagem(self):
        r = self._post(erro=llm_cli.ErroCLI("claude terminou com erro 1: Not logged in"))
    ...
    def test_status_informa_quem_esta_instalado(self):
        ...
        self.assertEqual({k: prov[k]["disponivel"] for k in llm_cli.PROGRAMAS},
                         {"cli_claude": True, "cli_codex": False, "cli_gemini": False})


@unittest.skipUnless(REAL, "teste real desligado (gasta tokens): defina CATEGORIZADOR_TESTE_CLI=1")
