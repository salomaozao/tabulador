...
variável de ambiente CATEGORIZADOR_USD_BRL (padrão 5,40).
"""
...
USD_BRL = float(os.getenv("CATEGORIZADOR_USD_BRL", "5.40"))
...
def consolidar(ultimas: int = 50) -> dict:
    ...
    if pasta.exists():
        for p in pasta.glob("*.json"):
            ...
            chamadas.append({
                ...
                "_ord": m.group(1) + p.name, "_dia": m.group(1)[:8], "arquivo": p.name,
    ...
    total, por_modelo, por_pergunta, por_dia = _novo(), {}, {}, {}
    for c in chamadas:
        _somar(total, c)
    ...
    return {
        ...
        "ultimas": [{k: v for k, v in c.items() if k not in ("_ord", "_dia")} for c in chamadas[:ultimas]],
    }
