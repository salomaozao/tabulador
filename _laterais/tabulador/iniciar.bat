...
chcp 65001 >nul
cd /d "%~dp0"
title Tabulador de respostas abertas

rem ---- ambiente Python proprio (fora do OneDrive, para nao sincronizar milhares de arquivos)
set "VENV=%LOCALAPPDATA%\Tabulador\venv"
set "VPY=%VENV%\Scripts\python.exe"

if exist "%VPY%" goto :dependencias

echo Preparando o Tabulador pela primeira vez (leva de 1 a 3 minutos)...
set "PY="
...
:rodar
echo.
echo Abrindo o Tabulador no navegador...
...
echo.
echo Python instalado. Feche esta janela e de dois cliques de novo no atalho do Tabulador.
pause
exit /b 0

:sem_winget
echo Baixe e instale o Python em https://www.python.org/downloads/
echo (na instalacao, marque "Add python.exe to PATH") e depois abra o Tabulador de novo.
...
echo   %VENV%
echo e abra o Tabulador de novo para reinstalar os pacotes.
pause
exit /b 1
